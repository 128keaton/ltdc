...
/***/
void ConvertToHeader(const char* inFile, const char* outFile, i32 posX, i32 posY, i32 sizeX, i32 sizeY, i32 numX, i32 numY, i32 colorNum, u32 transColor, const char* name, Compressor comp)
{
	FIBITMAP *dib, *dib32;
	...
	if(dib != NULL)
	{
		...
		// Build header file
		outData += "// Sprite table generated by MSXImage "VERSION"\n";
		outData += "const u8 ";
		...
		for(ny = 0; ny < numY; ny++)
		{
			for(nx = 0; nx < numX; nx++)
			{
				...
				{
					sprintf_s(tempData, 1024, "\t0x%02X, 0x%02X, 0x%02X, 0x%02X, // minX, minY, sizeX, sizeY \n", minX, minY, 1 + maxX - minX, 1 + maxY - minY);
					outData += tempData;
					totalBytes += 4;
				}
				else if(comp == COMPRESS_Crop16)
				{
					sprintf_s(tempData, 1024, "\t0x%02X, 0x%02X, // minX|minY, sizeX|sizeY \n", (minX << 4) + minY, ((1 + maxX - minX) << 4) + (1 + maxY - minY));
					outData += tempData;
					totalBytes += 2;
				}
				for(j = 0; j < sizeY; j++)
		...
		sprintf_s(tempData, 1024, "// Total size: %i bytes", totalBytes);
		outData += tempData;
...
int main(int argc, const char* argv[])
{
	...
	if(inFile && outFile)
	{
		...
		{
			ConvertToHeader(inFile, outFile, posX, posY, sizeX, sizeY, numX, numY, colorNum, transColor, name, comp);
		}
		else
		{
