...
void SendHttpRequest()
{
    ...
    SendLineToTcp(TcpOutputData);
    sprintf(TcpOutputData, "User-Agent: HGET/1.0 (MSX-DOS %s; TCP/IP UNAPI; %s)\r\n", dosVersion, unapiImplementationName);
    SendLineToTcp(TcpOutputData);
...
void ExtractHeaderTitleAndContents()
{
    ...
    *pointer = '\0';
    ...
    SkipCharsUntil(pointer, (char)13);
    *pointer = '\0';
...
void CheckHeaderErrors()
{
    if(contentLength == 0 && !isChunkedTransfer) {
